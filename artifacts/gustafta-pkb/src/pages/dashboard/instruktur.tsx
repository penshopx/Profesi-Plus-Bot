      ...
      <header className="border-b border-border bg-card px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">Dashboard Instruktur</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-medium">{user?.fullName}</p>
            <p className="text-[11px] text-muted-foreground">Instruktur</p>
          </div>
          <button onClick={() => navigate("/videos")} className="text-xs px-3 py-1.5 rounded-lg bg-muted hover:bg-muted/80 text-muted-foreground transition-colors">Video Library</button>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-0.5">Kelola materi video untuk peserta</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div className="bg-card border border-border rounded-2xl p-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Total Video</p>
          </div>
        </div>

        {/* Video list */}
        <div>
          <h2 className="text-sm font-semibold mb-3">Video Saya</h2>
          {isLoading ? (
            ...
            <div className="space-y-2">
              {videos.map((v: any) => (
                <div key={v.id} className="bg-card border border-border rounded-xl px-4 py-3 flex items-center gap-3 group">
                  ...
                  <a href={v.url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              {[
                { key: "title", label: "Judul Video *", placeholder: "Contoh: SKK Ahli Muda Teknik Gedung" },
                { key: "url", label: "URL Video *", placeholder: "https://youtube.com/watch?v=..." },
                { key: "skkUnitCode", label: "Kode SKK Unit", placeholder: "Contoh: M.711000.001.01" },
                { key: "skkUnitName", label: "Nama SKK Unit", placeholder: "Contoh: Menerapkan SMKK" },
                { key: "description", label: "Deskripsi", placeholder: "Deskripsi singkat video..." },
                { key: "tags", label: "Tags", placeholder: "Contoh: K3, SMKK, konstruksi" },
              ...
              <div>
                ...
                <select
                  value={form.jabker}
                  onChange={e => setForm(f => ({ ...f, jabker: e.target.value }))}
                  className="w-full rounded-xl border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                >
            ...
            <div className="flex gap-2.5 mt-5">
              <button onClick={() => setShowForm(false)} className="flex-1 rounded-xl border border-border py-2.5 text-sm font-medium hover:bg-muted transition-colors">
                Batal
              </button>
              <button
                onClick={() => addMut.mutate(form)}
                disabled={!form.title || !form.url || addMut.isPending}
