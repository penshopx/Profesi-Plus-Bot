...
| EAS project configured | Run `eas init` and add the resulting `projectId` UUID to `app.json > extra.eas.projectId` |
...
> **One-time EAS setup**: After running `eas init`, add the project ID to `app.json`:
> ```json
> "extra": {
>   "eas": { "projectId": "<your-uuid-here>" }
> }
> ```
